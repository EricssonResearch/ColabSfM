...
from omnireg.metrics import compute_inlier_ratio, compute_mnn_inlier_ratio
from omnireg.utils import get_correspondences
...
class NLLLoss(nn.Module):
    ...
    def forward(self, data):
        ...
        if omnireg.LOGGER is not None and (omnireg.GLOBAL_STEP % 10 == 0):
            omnireg.LOGGER.add_scalars('data/train_loss', {"global_nll": self.tracked_metrics[f"global_mean_neg_nll_{i}"], 
                                                           "global_bce": self.tracked_metrics[f"global_mean_bce_{i}"]}, global_step = omnireg.GLOBAL_STEP)
            omnireg.LOGGER.add_scalars('data/train_loss', {"refine_nll": self.tracked_metrics[f"refine_mean_neg_nll_{i}"], 
                                                           "refine_bce": self.tracked_metrics[f"refine_mean_bce_{i}"]}, global_step = omnireg.GLOBAL_STEP)
            omnireg.LOGGER.add_scalars('data/train_metrics', {"refine_IR": self.tracked_metrics.get(f"refine_IR", 0),
                                                              "global_IR": self.tracked_metrics[f"global_IR"]}, global_step = omnireg.GLOBAL_STEP)
        return tot_loss
