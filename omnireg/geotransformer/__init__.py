...
from omnireg.geotransformer.modules.sinkhorn import LearnableLogOptimalTransport
from omnireg.geotransformer.modules.geotransformer import (
...
from omnireg.geotransformer.modules.kpconv import ConvBlock, ResidualBlock, UnaryBlock, LastUnaryBlock, nearest_upsample


class GeoTransformer(nn.Module):
    def __init__(self, cfg, bckb="3dmatch"):
        ...
        if bckb == "3dmatch":
            from .geotr3d_backbone import KPConvFPN
        else:
            from .geotrkit_backbone import KPConvFPN
    ...
    def forward(self, data_dict):
        ...
        _, ref_node_masks, ref_node_knn_indices, ref_node_knn_masks = point_to_node_partition(
            ref_points_f, ref_points_c, min(self.num_points_in_patch, ref_points_f.shape[0])
        )
        _, src_node_masks, src_node_knn_indices, src_node_knn_masks = point_to_node_partition(
            src_points_f, src_points_c, min(self.num_points_in_patch, src_points_f.shape[0])
        )
        ...
        if self.debug: print("\tFeature shapes", ref_points_c.shape, src_points_c.shape, ref_feats_c.shape, src_feats_c.shape)
...
def create_model(config, backbone="3dmatch"):
    b = backbone.lower()
    print("Selected model:", b)
    assert b in ["3dmatch", "kitti"], "Backbone should be 3dmatch or kitti, got %s"%b
    model = GeoTransformer(config, bckb=backbone)
    return model
