...
# from omnireg.roitr.lib.utils import get_correspondences
...
class CoarseMatchingLoss(nn.Module):
    ...
    def forward(self, output_dict):
        ...
        if omnireg.LOGGER is not None:
            omnireg.LOGGER.add_scalars('data/roitr_train_loss', {"circle_loss": loss.item()}, global_step = omnireg.GLOBAL_STEP)
        return loss
...
class OverallLoss(nn.Module):
    ...
    def forward(self, output_dict, data_dict):
        if 'src_node_feats' in output_dict:
            ...
            if omnireg.LOGGER is not None:
                omnireg.LOGGER.add_scalars('data/roitr_train_loss', {"fine_loss": fine_loss.item()}, global_step = omnireg.GLOBAL_STEP)
            omnireg.GLOBAL_STEP = omnireg.GLOBAL_STEP + 1
        else:
