...
def get_dataset(config):
    ...
    if config.dataset == 'tdmatch':
        ...
        info_benchmark = load_info(f'omnireg/roitr/configs/tdmatch/{config.benchmark}.pkl')
    ...
    elif config.dataset in ['sfmreg','quad']:
        info_train = load_info(config.train_info)
        info_val = load_info(config.val_info)
        info_benchmark = load_info(config.test_info)

        training_set = SFMRegDataset(info_train, config, 'train', data_augmentation=True)
        val_set = SFMRegDataset(info_val, config, 'val', data_augmentation=False)
        testing_set = SFMRegDataset(info_benchmark, config, 'test', data_augmentation=False)
