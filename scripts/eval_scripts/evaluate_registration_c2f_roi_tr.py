...
def benchmark_registration(desc, exp_dir, whichbenchmark, n_points, ransac_with_mutual=False, inlier_ratio_threshold=0.05):
    gt_folder = f'third_party/RoITr/configs/benchmarks/{whichbenchmark}'
    ...
    coarse_sample = 256
    idx = 0
    for eachfile in tqdm(np.random.permutation(desc)):
        ...
        rot, trans = data['rot'].cpu(), data['trans'].cpu()
        src_corr_pts, tgt_corr_pts = data['src_corr_pts'].cpu(), data['tgt_corr_pts'].cpu()
        confidence = data['confidence'].cpu()
        ######################################################
        # 2. run ransac
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source_path', default="eval_cache", type=str, help='path to precomputed features and scores')
