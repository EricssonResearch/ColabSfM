...
if __name__ == "__main__":
    ...
    for mega_scene in mega_scenes:
        if "50" in mega_scene:
            print(f"Not doing {mega_scene} for now")
            continue
        scene_path = Path(f'datasets/sfmreg/reconstructions/sfm/{mega_scene}')
        if os.path.exists(scene_path):
            if len(os.listdir(scene_path)) == num_subsets:
                print(f"Done with {mega_scene}")
                continue
        ...
        for subset in range(num_subsets):
            outputs = Path(f'datasets/sfmreg/reconstructions/sfm/{mega_scene}/{subset}')
            if os.path.exists(outputs):
                print(f"Path {outputs} already e[xists, skipping")
            ...
            try:
                ...
                pairs_from_retrieval.main(retrieval_path, sfm_pairs, num_matched= min(40, len(image_ids)))
