...
for scene in scenes:
    ...
    for i in list(sorted(partial_reconstructions)):
        try:
            ...
            np.save(f"{pointcloud_dir}/{scene}/{method}_cloud_{i}.npy",pts[-1])
            np.save(f"{pointcloud_dir}/{scene}/{method}_viewpoints_{i}.npy", viewpoints)
        except:
    ...
    for i in partial_reconstructions:
        for j in partial_reconstructions:
            ...
            IOU = (overlap_12.sum() / len(pts1)) * (overlap_21.sum() / len(pts2))
            IOU_mat[i,j] = IOU
