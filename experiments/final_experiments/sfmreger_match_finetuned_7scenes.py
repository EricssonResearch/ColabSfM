...
if __name__ == '__main__':
    ...
    for scene in scenes:
        points_A = np.load(f"data/sfmreg/7scenes/{scene}_benchmark/pointclouds/Train/{method}_cloud.npy")
        viewpoints_A = np.load(f"data/sfmreg/7scenes/{scene}_benchmark/pointclouds/Train/{method}_viewpoints.npy")
        rot_ab = np.eye(3)
        tr_ab = np.zeros([3])
        if True:
