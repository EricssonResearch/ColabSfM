...
def main(points_A, points_B, viewpoints_A = None, viewpoints_B = None, features_A = None, features_B = None, shared_scale = True, gt = None):
    ...
    config.pretrain = 'workspace/y24w9_sfmreg_megadepth_color_old/model_best_PIR.pth'
    # config.pretrain = 'pretrained/omnireg.pth'
...
if __name__ == '__main__':
    ...
    for scene in scenes:
        ...
        features_A = np.load(f"data/sfmreg/cambridge/{scene}_benchmark/pointclouds/train/{method}_colors.npy").astype(np.float32)/255.
        rot_ab = np.eye(3)
        tr_ab = np.zeros([3])
        if True:
        ...
        features_B = np.load(f"data/sfmreg/cambridge/{scene}_benchmark/pointclouds/test/{method}_colors.npy").astype(np.float32)/255.
