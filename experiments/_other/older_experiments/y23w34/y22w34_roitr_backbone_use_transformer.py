...
def train(args):
    ...
    config = edict(normalize_descriptors = False, backbone = 'roitr', root="data/indoor", overlap_radius = 0.0375, augment_noise = 0.005, encoder_only = True, use_global_transformer = True)
    model = OmniGlue(
        config,
        ).to(get_best_device())
