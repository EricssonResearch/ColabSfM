...
def train(args):
    ...
    config = edict(normalize_descriptors = False, backbone = 'roitr', root="data/indoor", 
                   overlap_radius = 0.0375, augment_noise = 0.005, encoder_only = False, 
                   use_global_transformer = True, nsample = [8, 16, 16, 16], use_refiner = True, normalize_features = False,
                   refiner_num_heads = 1, refiner_n_layers = 4, refiner_descriptor_dim = 256, refine_num_nn = 8, refiner = "local_softmax")
    model = OmniGlue(
        config,
        ).to(get_best_device())
