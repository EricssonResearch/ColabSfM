...
sys.path.append('third_party/BUFFER/')
sys.path.append('experiments/evaluate_buffer/')
import os
...
from dataloader import collate_fn_descriptor
from dataset import convert_pointcloud
...
if __name__ == '__main__':
    ...
    batch = {}
    model.eval()
    src_points = np.load("experiments/evaluate_buffer/depth.npy").astype(np.float32)
    ref_points = np.load("experiments/evaluate_buffer/lidar.npy").astype(np.float32)
    list_data = convert_pointcloud(src_points, ref_points, cfg)
    collate_fn_descriptor(list_data, cfg, neighborhood_limits = 1) # TODO: original work uses quite complex func to compute neighbourhood_limits
    batch["src_pcd"] = src_points
    batch["src_pcd_raw"] = src_points
    batch["tgt_pcd"] = ref_points
    batch["tgt_pcd_raw"] = ref_points
    batch["stack_lengths"] = [[1000]]
    batch["relt_pose"] = np.eye(4, dtype = np.float32) 
