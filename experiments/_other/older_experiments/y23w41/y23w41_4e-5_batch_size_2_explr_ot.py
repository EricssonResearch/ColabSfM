...
def train(args):
    ...
    config = edict(normalize_descriptors = False, backbone = 'roitr', root=f"{args.data_root}/indoor", 
                   overlap_radius = 0.0375, augment_noise = 0.005, encoder_only = False, n_layers = 1,
                   use_global_transformer = True, nsample = [8, 16, 16, 16], use_refiner = True, normalize_features = False,
                   refine_level = 1, refine_num_nn = 64, refiner = "OT", num_iter = 100,
                   ppf_mode = "baseline",
    ...
    lr_scheduler = ExponentialLR(gamma = 0.95, optimizer=optimizer)
    objective = NLLLoss()
    num_epochs = 150
    num_gpus = 1
