...
def train(args):
    ...
    config = edict(normalize_descriptors = False, backbone = 'roitr', root=f"{args.data_root}/indoor", colabsfm_root="data/colabsfm/pointclouds",
                   ...
                   refiner_num_heads = 1, refiner_n_layers = 8, refine_level = 3, refiner_descriptor_dim = 256, refine_num_nn = 8, refiner = "local_softmax",
                   ppf_mode = "no_dists",
    ...
    dataset = MegaDepth(data_root= "data", infos = load_obj("data/colabsfm/pointclouds/sift_infos"), config = config, max_points = 16_000)
    tdmatch_benchmark = TDMatchBenchmark(IndoorDataset(data_root= args.data_root, 
                                                       infos = load_obj(f"{args.data_root}/indoor/3DLoMatch.pkl"), 
                                                       config = config,
                                                       max_points=30_000))
    batch_size = 1
    ...
    import numpy as np
    #model.match(torch.from_numpy(np.load("data/colabsfm/0100/sift_cloud_0.npy")).cuda(), torch.from_numpy(np.load("data/colabsfm/0100/sift_cloud_1.npy")).cuda())
    
    #model.match(torch.from_numpy(np.load("0100/sift_cloud_0.npy")).cuda(), torch.from_numpy(np.load("0100/sift_cloud_1.npy")).cuda())
    if args.colabsfm_benchmark:
        import json
        colabsfm_benchmark = TDMatchBenchmark(MegaDepth(data_root= "data/colabsfm/pointclouds", infos = load_obj("data/colabsfm/pointclouds/sift_infos"), config = config, max_points = 30_000))
