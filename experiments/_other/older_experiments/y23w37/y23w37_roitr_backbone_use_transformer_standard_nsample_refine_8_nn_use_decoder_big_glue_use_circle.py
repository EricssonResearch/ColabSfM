...
def train(args):
    ...
    config = edict(normalize_descriptors = False, backbone = 'roitr', root=f"{args.data_root}/indoor", 
                   ...
                   refiner_num_heads = 1, refiner_n_layers = 8, refiner_descriptor_dim = 256, refine_num_nn = 8, refiner = "local_softmax",
                   signature = experiment_name)
    model = OmniGlue(
        config,
        ).to(get_best_device())
    #model = nn.DataParallel(model)
    dataset = IndoorDataset(data_root= args.data_root, infos = load_obj(f"{args.data_root}/indoor/train_info.pkl"), config = config)
    ...
    circle_loss = CoarseMatchingLoss(circle_loss_config)
    objective = WeightedCombination([nll_loss, circle_loss], [1., 5.])    
    num_epochs = 150
    num_gpus = 1
