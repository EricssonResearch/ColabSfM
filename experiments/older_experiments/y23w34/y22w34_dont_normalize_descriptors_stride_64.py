...
def train(args):
    ...
    config = edict(root="data/indoor", overlap_radius = 0.0375, augment_noise = 0.005, strides = [1, 2, 2, 2, 2, 4], normalize_descriptors=False, backbone = "omniglue", scale_invariant = False)
    model = OmniGlue(config).to(get_best_device())
