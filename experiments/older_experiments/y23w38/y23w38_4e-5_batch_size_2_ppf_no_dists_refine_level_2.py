...
def train(args):
    ...
    config = edict(normalize_descriptors = False, backbone = 'roitr', root=f"{args.data_root}/indoor", sfmreg_root=f"{args.data_root}/sfmreg/0015",
    ...
    if args.sfmreg_benchmark and False:
        from colabsfm.datasets.sfmreg import SFMRegDataset
        import json
        sfmreg_benchmark = TDMatchBenchmark(SFMRegDataset(data_root= "data/sfmreg", infos = json.load(open("data/sfmreg/0015/disk+lightglue_infos.json","r")), config = config))
