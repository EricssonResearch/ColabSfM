...
class GlobalMatcher(nn.Module):
    def __init__(self, conf, **kwargs) -> None:
        ...
        self.log_assignment = nn.ModuleList(
            [MatchAssignment(d, normalize_descriptors = conf.normalize_descriptors) for _ in range(n)])
        self.token_confidence = nn.ModuleList([
            TokenConfidence(d) for _ in range(n-1)])

    def forward(self, data):
        desc0, desc1 = data['padded_descriptor_A'] if 'pad_mask_A' in data else data['descriptor_A'], data['padded_descriptor_B'] if 'pad_mask_B' in data else data['descriptor_B']
        mask0, mask1 = data['pad_mask_A'] if 'pad_mask_A' in data else None, data['pad_mask_B'] if 'pad_mask_B' in data else None
        ...
        for i in range(self.conf.n_layers):
            ...
            scores, matchability_A, matchability_B = self.log_assignment[i](desc0, desc1, mask0 = mask0, mask1 = mask1)
            all_scores.append(scores)
            all_matchability.append((matchability_A[...,0], matchability_B[...,0]))
        data['scores'] = all_scores
        data['matchability'] = all_matchability
        return data
