...
if __name__ == "__main__":
    ...
    scene_path = Path(base_path).joinpath(Path('reconstructions/sfm_trajectory/'))
    # if os.path.exists(scene_path):
    #     raise ValueError(f"{scene_path} exists, skipping.")
        
    print("Creating partial reconstructions")
    reference_model_path = base_path / sfm / 'sift'
    ...
    qvecs = np.array([im.cam_from_world.rotation.quat for im in ref_reconstruction.images.values()])
    ...
    try:
        if os.path.exists(scene_path / 'trajectories.pkl'):
            ...
            print("Loaded trajectories for")
        else:
            trajectories = generate_trajectories(positions, position_distances, rot_distances, max_num_trajectories = 30)
