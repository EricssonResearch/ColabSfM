...
from .model import *
from .modules import CoarseMatching, AdaptiveSuperPointMatching, GTCoarseCorrGenerator, FineMatching, LearnableLogOptimalTransport
...
class RIGA_v2(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.benchmark in ['3DMatch', '3DLoMatch', 'colabsfm']:
        ...
        self.backbone = RIPointTransformer(transformer_architecture=config.transformer_architecture, c = 3 if config.use_color else 1,
                                           with_cross_pos_embed=self.with_cross_pos_embed, factor=self.factor, ppf_mode=config.ppf_mode)
        # learnable Optimal Transport Layer
        self.OT = LearnableLogOptimalTransport(num_iter=1)
        ...
        if config.use_glue:
            from .glue import GlobalGlue
            self.glue = GlobalGlue(config)
        
        if config.glue_refine:
            from .refine_glue import Refiner
            self.refine_glue = Refiner(config)
            self.fine_proj = nn.Linear(64*self.factor, config.refiner_descriptor_dim*self.factor)
    ...
    def forward(self, src_pcd, tgt_pcd, src_feats, tgt_feats, src_normals, tgt_normals, rot, trans, src_raw_pcd):
        ...
        desc0, desc1 = self.coarse_proj(src_node_feats), self.coarse_proj(tgt_node_feats)
        if self.config.use_glue:
            desc0, desc1 = self.glue(desc0, desc1)
        src_node_feats = F.normalize(desc0, p=2, dim=1)
        tgt_node_feats = F.normalize(desc1, p=2, dim=1)
        ...
        with torch.no_grad():
            ...
            #print(f"{node_corr_scores.mean()=}")
            #print(f"{node_corr_scores.std()=}")
        ...
        # 5. Optimal transport
        if self.config.glue_refine:
            desc0, desc1 = self.refine_glue(tgt_node_corr_knn_feats, src_node_corr_knn_feats)
        else:
            desc0, desc1 = tgt_node_corr_knn_feats, src_node_corr_knn_feats 
