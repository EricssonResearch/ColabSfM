...
from .transformer.ppftransformer import LocalPPFTransformer
from .transformer.geotransformer import GeometricTransformer

class RIPointTransformerLayer(nn.Module):
    '''
    Rotation-invariant point transformer layer
    '''
    def __init__(self, in_planes, out_planes, num_heads=4, nsample=16, factor=1, ppf_mode = "baseline"):
        super().__init__()
        ...
        self.ppf_mode = ppf_mode
        if ppf_mode in ("symmetric", "flip_to_convex"):
            self.ppf_proj = nn.Sequential(nn.Linear(4,32), nn.ReLU(inplace=True), nn.Linear(32,4))
    ...
    def forward(self, pxon, mask=None) -> torch.Tensor:
        ...
        p_r = p[group_idx, :]
        n_r = n[group_idx, :]
        #assert ppf_r is None, "where did ppf come from?"
        if ppf_r is None:
            ppf_r = calc_ppf_gpu(p, n, p_r, n_r)  # (n, nsample, 4)
            if self.ppf_mode == "baseline":
                pass
            elif self.ppf_mode == "flip_to_convex":
                flip = ppf_r.mean(dim=1)[:,2] > 0.5 # Decide whether to flip
                ppf_r[flip,:,1:3] = 1 - ppf_r[flip,:,1:3]
            elif self.ppf_mode == "flip_to_convex_no_dist":
                flip = ppf_r.mean(dim=1)[:,2] > 0.5 # Decide whether to flip
                ppf_r[flip,:,1:3] = 1 - ppf_r[flip,:,1:3]
                # set dist to 0
                ppf_r[:,:,0] = 0
            elif self.ppf_mode == "symmetric":
                ppf_pos = ppf_r
                ppf_neg = ppf_pos.clone()
                ppf_neg[:,:,1:3] = 1 - ppf_neg[:,:,1:3]
                ppf_r = self.ppf_proj(ppf_neg) + self.ppf_proj(ppf_pos) 
            elif self.ppf_mode == "no_dist":
                ppf_r[...,0] = 0 
...
class TransitionDown(nn.Module):
    ...
    Down-sampling
    '''
    def __init__(self, in_planes, out_planes, num_heads=4, stride=1, nsample=16, factor=1, ppf_mode = "baseline"):
        super().__init__()
    ...
    def forward(self, pxon):
        ...
        if self.ppf_mode == "baseline":
            ...
            flip = ppf.mean(dim=1)[:,2] > 0.5 # Decide whether to flip
            ppf[flip,:,1:3] = 1 - ppf[flip,:,1:3]
        elif self.ppf_mode == "flip_to_convex_no_dist":
            flip = ppf.mean(dim=1)[:,2] > 0.5 # Decide whether to flip
            ppf[flip,:,1:3] = 1 - ppf[flip,:,1:3]
            # set dist to 0
            ...
            ppf_pos = ppf
            ppf_neg = ppf_pos.clone()
            ppf_neg[:,:,1:3] = 1 - ppf[:,:,1:3]
            ppf = self.ppf_proj(ppf_neg) + self.ppf_proj(ppf_pos)
        elif self.ppf_mode == "no_dist":
            ppf[...,0] = 0        
...
class RIPointTransformerBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, num_heads=4, nsample=16, factor=1, ppf_mode = "baseline"):
        super(RIPointTransformerBlock, self).__init__()
        self.transformer = RIPointTransformerLayer(in_planes, planes, num_heads, nsample, factor, ppf_mode = ppf_mode)
...
class RIPointTransformer(nn.Module):
    def __init__(self, blocks=[2, 3, 3, 3], block=RIPointTransformerBlock, c=1, transformer_architecture=None, with_cross_pos_embed=None, 
                 factor=1, occ_thres=0., encoder_only = False, use_global_transformer = True, nsample = None, return_dictionary = False, descriptor_dim = 256, ppf_mode = "baseline"):
        super().__init__()
        self.ppf_mode = ppf_mode
        self.c = c
        self.num_heads = 4
        self.in_planes, planes = c, [64*factor, 128*factor, 256*factor, descriptor_dim*factor]
        stride = [1, 4, 4, 4]
        if nsample is None:
            nsample = [8, 16, 16, 16]
        ...
        self.global_transformer = GeometricTransformer(descriptor_dim*factor, descriptor_dim*factor, descriptor_dim*factor, 
                                                       4, self.transformer_architecture, sigma_d=0.2, sigma_a=15, angle_k=3, ppf_mode = ppf_mode)
    ...
    def _make_enc(self, block, planes, blocks, num_heads=4, stride=1, nsample=16, factor=1):
        layers = []
        layers.append(TransitionDown(self.in_planes, planes, num_heads, stride, nsample, factor, ppf_mode = self.ppf_mode))
        self.in_planes = planes
        for _ in range(1, blocks):
            layers.append(block(self.in_planes, self.in_planes, num_heads, nsample=nsample, factor=factor, ppf_mode = self.ppf_mode))
    ...
    def forward(self, s_pxon, t_pxon, src_deformed_pcd):
        ...
        if not self.use_global_transformer:
            s_g_x4, t_g_x4 = s_x4, t_x4
        else:
            ###########################
