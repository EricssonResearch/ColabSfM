...
from colabsfm.pointops import knnquery
import glob, os
...
def get_node_correspondences(
    ...
    src_nodes = torch.matmul(src_nodes, rot.T) + trans[None]
    src_knn_points = torch.matmul(src_knn_points, rot.T) + (trans)[None]#[None, ::]
    ...
    node_mask_mat = ref_masks.unsqueeze(1) * src_masks.unsqueeze(0)  # (M, N)
    ...
    point_mask_mat = ref_knn_masks.unsqueeze(2) * src_knn_masks.unsqueeze(1)  # (B, K, K)
