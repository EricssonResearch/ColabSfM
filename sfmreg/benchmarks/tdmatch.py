...
from sfmreg.metrics import compute_feature_matching_recall, compute_inlier_ratio
from sfmreg.models.model import OmniGlue
from sfmreg.utils import get_best_device, collate_ragged
...
class TDMatchBenchmark:
    ...
    def benchmark(self, model: OmniGlue, device = get_best_device(), refiner = True):
        ...
        if sfmreg.LOGGER is not None:
            sfmreg.LOGGER.add_scalars('data/3dmatch_benchmark', 
                                       dict(LO_FMR = LO_FMR, LO_IR = LO_IR, FMR = FMR, IR = IR, mean_num_matches = mean_num_matches), 
                                       global_step = sfmreg.GLOBAL_STEP)
        else:
